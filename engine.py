...
# --- NETWORK IMPORT ---
# We wrap this in a try/except block just in case network.py is missing,
# but for your usage, it should be there.
try:
    import network
    NETWORK_AVAILABLE = True
except ImportError:
    print("⚠️ WARNING: 'network.py' not found. Mobile features disabled.")
...
from terrapolis_logic import TerrapolisGame, MAP_H, MAP_W


# --- ID MAPPING FOR MOBILE PROTOCOL ---
# This dictionary maps building names to integer IDs expected by the mobile app.
BUILDING_TO_ID = {
    'plain': 1,
    'forest': 2,
    'mountain': 3,
...
class Game:
    def __init__(self):
        ...
        # --- NETWORK INITIALIZATION (FROM ENGINE2) ---
        self.network = None
        if NETWORK_AVAILABLE:
            try:
                # We pass 'self' so the server can access game logic if needed
                self.network = network.TerrapolisServer(self)
                print("✅ [NETWORK] Mobile Server Started on Port 5005")
    ...
    def _init_display(self):
        ...
        pygame.display.set_caption("City Builder - Terrapolis AR + Mobile")
    ...
    def reset_game(self):
        ...
        # Sync initial with mobile
        self._send_map_to_mobile()
    ...
    # =========================================================================
    # === NETWORK & MOBILE METHODS (FUSION FROM ENGINE2) ===
    # =========================================================================

    def _get_game_state_string(self):
        """Prepares map data string for mobile app (with -90 deg rotation)."""
        grid_for_mobile = np.zeros((cfg.MAP_HEIGHT, cfg.MAP_WIDTH), dtype=int)

        for r in range(cfg.MAP_HEIGHT):
            for c in range(cfg.MAP_WIDTH):
                # Base terrain or building ID
                tile_type = self.buildings_grid[r][c] if self.buildings_grid[r][c] else self.map_data[r][c]
                val = BUILDING_TO_ID.get(tile_type, 1) # Default 1 (plain)
                
                # Flood overrides visuals
                if self.flooded_grid[r][c]:
                    val = 99
                
                grid_for_mobile[r, c] = val

        # Rotate -90 degrees for vertical phone screen layout
        rotated_grid = np.rot90(grid_for_mobile, k=-1)
        
        # Convert to CSV-like string: "1,1,2;4,5,6..."
        rows_str = []
    ...
    def _send_map_to_mobile(self):
        """Broadcasts current game state to mobile clients."""
        if self.network:
            try:
                data = self._get_game_state_string()
                # Protocol: MAP|GridData|Wood|Stone|Energy|Pollution
                # Note: Energy is technically Virtuosity in your model, mapping it here.
                full_msg = f"MAP|{data}|{int(self.resources['wood'])}|{int(self.resources['stones'])}|{int(self.resources['virtuosity'])}|{int(self.final_stats.get('pollution_total', 0))}"
                self.network.broadcast(full_msg)
            except Exception as e:
                print(f"Error sending to mobile: {e}")

    def _process_network_commands(self):
        """Processes incoming commands from the mobile app."""
        if not self.network: return
        
        while not self.network.command_queue.empty():
            cmd_data = self.network.command_queue.get()
            cmd = cmd_data.get("command", "")
            
            if cmd == "READY":
                print("📱 [MOBILE] Connected! Sending map sync...")
                self._send_map_to_mobile()
            
            elif cmd == "BUILD":
                # Mobile sends (x,y) based on rotated grid.
                # We must rotate coordinates back: (row, col) = (mobile_y, mobile_x)
                r, c = cmd_data["y"], cmd_data["x"]
                b_type = cmd_data["type"]
                
                print(f"📱 [MOBILE] Order: Build {b_type} at ({c},{r})")
                
                # We use existing logic to place building
                # NOTE: Mobile bypasses selection ghost, acts immediately
                self.selected_building = b_type
                
                # Check constraints manually since place_building usually relies on mouse pos
                rules = BUILDING_RULES.get(b_type)
                if rules and self._check_building_constraints(c, r, rules) and self._check_resources_cost(rules):
                    self.execute_action(c, r, b_type, 1)
                    self._send_map_to_mobile() # Sync back
                else:
                    self.network.send_popup("ERREUR", "Construction Impossible", "Zone invalide ou ressources manquantes.")
                
                self.selected_building = None # Reset selection

            elif cmd == "DESTROY":
                r, c = cmd_data["y"], cmd_data["x"]
                print(f"📱 [MOBILE] Order: Destroy at ({c},{r})")
                
                # Execute destroy logic
                b_name = self.buildings_grid[r][c]
                if b_name:
                    self.execute_action(c, r, b_name, -1)
                    self._send_map_to_mobile()
    ...
    def _end_game(self):
        ...
        # Sync Mobile on End Game
        self._send_map_to_mobile()
        if self.network:
            self.network.send_popup("INFO", "FIN DE PARTIE", f"Score Final: {int(score)}")
    ...
    def _handle_flood_timers(self, dt):
        ...
        if self.flood_clear_timer > 0:
            ...
            if self.flood_clear_timer <= 0:
                ...
                self._send_map_to_mobile() # Sync on flood clear
    ...
    def _process_production_cycle(self):
        ...
        # Periodic Sync to update resources on mobile
        # Optimization: Only sync if values changed significantly or every N seconds
        # For now, simplistic approach:
        # self._send_map_to_mobile() # CAUTION: Doing this every second might flood network. Kept sparse.
    ...
    def _destroy_terrain_resource(self, x, y):
        ...
        self.map_data[y][x] = "plain"
        self._send_map_to_mobile() # Sync visual change
    ...
    def trigger_flood(self):
        ...
        self._send_map_to_mobile() # Sync
        if self.network:
            self.network.send_popup("DANGER", "INONDATION !", f"{len(flooded_selection)} zones touchées.")
    ...
    def trigger_popup(self, type, title, message, action_data=None):
        self.popup_active = True
        self.popup_data = {"type": type, "title": title, "message": message, "action": action_data}
        
        # Send to mobile as well
        if self.network:
            # Map types to simple levels for mobile
            lvl = "INFO"
            if type == "error": lvl = "ERREUR"
            elif type == "confirm": lvl = "QUESTION"
            self.network.send_popup(lvl, title, message)
